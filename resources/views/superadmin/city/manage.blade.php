         ...
         <div class="card">
            <div class="card-body table-responsive">
               <table class="table table-striped" id="city">
                ...
                <a href="{{url('superadmin/city')}}" type="button" class="btn btn-info" style="float: left; position: relative; left: 10px;">Add New</a>
                  <thead>
                     <tr>
                        <th>S.No</th>
                        <th>City</th>
                  ...
                  <tbody>
                     ...
                     <tr>
                        ...
                        <td>{{$slider->lat}}</td>
                        <td>{{$slider->long}}</td>
                        <!-- <td>{{$slider->buisness}}</td>
                        <td>{{$slider->promo_available}}</td>
                        <td>{{$slider->pay_by_cash}}</td>
                        <td>{{$slider->other_payment_method}}</td>
                        <td>{{$slider->wallet_in_cash}}</td>
                        <td>{{$slider->wallet_in_other}}</td>
                        <td>{{$slider->received_cash_payment}}</td>
                        <td>{{$slider->queries}}</td> -->
  ...
  $(document).ready(function(){
   ...
   $(document).ready(function(){
      ...
      function filter(status = '') {
          $.ajax({
              url:"{{ url('superadmin/filter_city')}}",
              method:"GET",
              data:{
                status:status
              },
