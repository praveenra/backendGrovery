         ...
         <div class="card">
            <div class="card-body table-responsive">
               <table class="table table-striped" id="area">
                ...
                <a href="{{url('superadmin/zone/create')}}" type="button" class="btn btn-info" style="float: left; position: relative; left: 10px;">Add New</a>
                  <thead>
                     <tr>
                        <th>S. No</th>
                        <th>Zone Name</th>
                  ...
                  <tbody>
                     ...
                     <tr>
                        ...
                        <td>{{$slider->zone_lat}}</td>
                        <td>{{$slider->zone_lon}}</td>
                        <td>{{$slider->zone_radius}}</td>
...
<div id="delete" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="custom-width-modalLabel" aria-hidden="true" style="display: none;">
    <div class="modal-dialog" style="width:55%;">
        <div class="modal-content">
            <form action="{{route('deleteZone')}}" method="post">
              @csrf
  ...
  $(document).ready(function(){
   ...
   $(document).ready(function(){
      ...
      function filter(status = '') {
          $.ajax({
              url:"{{ url('superadmin/filter_zone')}}",
              method:"GET",
              data:{
                status:status
              },
