...
<div class="content-wrapper">
   <div class="page-header">
      <h3 class="page-title">Delivery Man Assign</h3>
      <nav aria-label="breadcrumb">
         <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="#">SuperAdmin</a></li>
            <li class="breadcrumb-item active" aria-current="page">Delivery Man Assign</li>
         </ol>
      </nav>
   </div>
   <div class="row">
            ...
            <div class="card-body table-responsive">
               ...
               <table class="table table-striped" id="orders">
                  ...
                  <tbody>
                     ...
                     @forelse($delivery_man as $key=>$slider)
                     ...
                     <tr>
                        ...
                        <td>
                            @if($slider->delivery_man == '')
                                <a class="delivery_man_check"  style="color: red; text-decoration: underline;" data-id="{{$slider->order_main_id}}" data-toggle="modal" data-target="#deliveryMan">Assign Delivery Man</a>
                            @else
                                {{$slider->delivery_boy_name}}
                            @endif
                        </td>

                        @if($slider->order_status == '')
                            <td><button class="btn-approve approve_check" data-id="{{$slider->order_main_id}}" data-toggle="modal" data-target="#approveOrReject">Approve Order</button></td>
                        @elseif($slider->order_status == 1)
                            <td><button class="btn-approve approve_check" data-id="{{$slider->order_main_id}}" data-toggle="modal" data-target="#approveOrReject">{{$slider->order_status_name}}</button></td>
                        @elseif($slider->order_status == 2)
                            <td><button class=" btn-progress progress_check delivery_man_assign_check" data-id="{{$slider->order_main_id}}" data-toggle="modal" data-target="#Ready_for_Pickup">{{$slider->order_status_name}}</button></td>
                        ...
                        @elseif($slider->order_status == 7)
                            <td><button class="btn-approve complete_check" data-id="{{$slider->order_main_id}}" data-toggle="modal" data-target="#complete_order">{{$slider->order_status_name}}</button></td>
                        @elseif($slider->order_status == 4)
                            <td><button class="btn-success return_check" data-id="{{$slider->order_main_id}}" data-toggle="modal" data-target="#return_order">{{$slider->order_status_name}}</button></td>
                        ...
                        @endif
                        <!-- <td>
                          <a href="javascript:;" class="order_details_modal" data-id="{{$slider->order_id}}" data-toggle="modal" data-target="#view" title="view"><i class="fa fa-eye"></i></a>&nbsp;&nbsp;
                        </td> -->
                     </tr>
                     @empty
                     <tr>
                  ...
                  <div class="row">
                    <div class="col-lg-8">
                        ...
                        <select class="form-control" name="delivery_partner" id="delivery_partner" style="margin-left: 70px;">
                            ...
                            <!-- @foreach($users as $user)
                                @if($user->order_status == 3 || $user->order_status == 5 || $user->order_status == 6)
                                    <option value="{{$user->id}}">{{$user->first_name}}</option>
                                @endif
                            @endforeach -->
...
<script type="text/javascript">
  ...
  $(document).ready(function(){
    $('.order_details_modal').click(function(){
        ...
        $.ajax({
            url: "{{url('superadmin/view_order')}}" + '/' + order_id,
            type: 'GET',
            dataType: 'json',
