                     ...
                     <tr>
                        ...
                        <td>
                            @if($slider->delivery_man == '')
                                <a class="delivery_man_check"  style="color: red; text-decoration: underline;" data-id="{{$slider->order_id}}" data-toggle="modal" data-target="#deliveryMan">Assign Delivery Man</a>
                            @else
                        ...
                        @if($slider->order_status == '')
                            <td><button class="btn-approve approve_check" data-id="{{$slider->order_id}}" data-toggle="modal" data-target="#approveOrReject">Approve Order</button></td>
                        @elseif($slider->order_status == 1)
                            <td><button class="btn-approve approve_check" data-id="{{$slider->order_id}}" data-toggle="modal" data-target="#approveOrReject">{{$slider->order_status_name}}</button></td>
                        @elseif($slider->order_status == 2)
                            <td><button class=" btn-progress progress_check delivery_man_assign_check" data-id="{{$slider->order_id}}" data-toggle="modal" data-target="#Ready_for_Pickup">{{$slider->order_status_name}}</button></td>

                        @elseif($slider->order_status == 3)
                           <td><button class="btn-pickup out_for_delivery_check" data-id="{{$slider->order_id}}" data-toggle="modal" data-target="#out_for_delivery_order">{{$slider->order_status_name}}</button></td>
                        @elseif($slider->order_status == 7)
                            <td><button class="btn-approve complete_check" data-id="{{$slider->order_id}}" data-toggle="modal" data-target="#complete_order">{{$slider->order_status_name}}</button></td>
                        @elseif($slider->order_status == 4)
                            <td><button class="btn-success return_check" data-id="{{$slider->order_id}}" data-toggle="modal" data-target="#return_order">{{$slider->order_status_name}}</button></td>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button type="xbmit" class="btn btn-danger waves-effect">Proceed</button>
                </div>
                </form>
            </div>
        </div>
...
<script type="text/javascript">
  ...
  $(document).ready(function(){
    $('.order_details_modal').click(function(){
        ...
        $.ajax({
            url: "{{url('admin/view_order')}}" + '/' + order_id,
            type: 'GET',
            dataType: 'json',
