<?php
	...
	class CartController extends Controller
	{
	   ...
	   public function maincategory()
	   {
		...
		//    echo "main-category =>", print_r($maincategorylists);
		   $finalarray=array();
	   ...
	   // Stores List with 2 Products under Main Category (Store Items)
		...
		public function storeReview(Request $request){
			...
			if($count == 0){
				...
				$review = Review::select('reviews.*','customer.*','reviews.id as review_id')->leftjoin('customer','customer.id','reviews.customer_id')->where('reviews.store_id',$sid)->where('reviews.status',1)->groupBy('reviews.id')->orderBy('reviews.id', 'desc')->get();
		...
		public function shopInformation(Request $request){
			...
			if($shop_timing){
				...
				return response()->json([
					'status'=>false,
					'message'=>'Store Settings not set'
				]);
			}			
		}

		public function shopInformationv1(Request $request){
			$seller_id = $request->sid;
			$seller = Seller::where('sd_id',$seller_id)->first();
			// echo "<pre> =>";
			// echo $seller;
			// exit;
			if($seller){
				$result = [];
				$data = [
					'business' => $seller->sunday_check,
					'opening_day' => 'sunday',
					'opening_time' => $seller->sunday_opening_time,
					'closing_time' => $seller->sunday_closing_time,
				];
				$result[] = $data;
				$data = [
					'business' => $seller->monday_check,
					'opening_day' => 'monday',
					'opening_time' => $seller->monday_opening_time,
					'closing_time' => $seller->monday_closing_time,
				];
				$result[] = $data;
				$data = [
					'business' => $seller->tuesday_check,
					'opening_day' => 'tuesday',
					'opening_time' => $seller->tuesday_opening_time,
					'closing_time' => $seller->tuesday_closing_time,
				];
				$result[] = $data;
				$data = [
					'business' => $seller->wednesday_check,
					'opening_day' => 'wednesday',
					'opening_time' => $seller->wednesday_opening_time,
					'closing_time' => $seller->wednesday_closing_time,
				];
				$result[] = $data;
				$data = [
					'business' => $seller->thursday_check,
					'opening_day' => 'thursday',
					'opening_time' => $seller->thursday_opening_time,
					'closing_time' => $seller->thursday_closing_time,
				];
				$result[] = $data;
				$data = [
					'business' => $seller->friday_check,
					'opening_day' => 'friday',
					'opening_time' => $seller->friday_opening_time,
					'closing_time' => $seller->friday_closing_time,
				];
				$result[] = $data;
				$data = [
					'business' => $seller->saturday_check,
					'opening_day' => 'saturday',
					'opening_time' => $seller->saturday_opening_time,
					'closing_time' => $seller->saturday_closing_time,
				];
				$result[] = $data;
			...
			$products = Products::select(
				...
				foreach ($products as $key => $data) {
					...
					$new_product[] = [
						...
						'product_tax' => $data['product_tax'],
						'product_price' => $product_quantity['price'],
						'product_offer' => number_format($product_quantity['offer'],2),
						'product_sales_price' => $product_quantity['sales_price'],
					];
				}
