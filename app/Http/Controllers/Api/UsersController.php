    ...
    class UsersController extends Controller
    {
      ...
      public function deviceid(Request $request)
            ...
            if ($token1->valid) {
                if ($token1->existsValid()) {
                    ...
                    if($id!=""){
                        ...
                        $customer_address->customer_lat = $request->customer_lat || 0.0;;
                        $customer_address->customer_lng = $request->customer_lng || 0.0;;
                    ...
                    if($id!=""){
                        ...
                        if($count == 0){
                           ...
                           if($sellers_count != 0){
                                ...
                                ->where('seller_details.sd_status',1)
        ...
        public function changeCartQuantity(Request $request)
        {
            ...
            else
            {
                ...
                if($quantity_count>1){
                    ...
                    $cart = Addtocart::where('id',$cart_id)->delete();
                }
        ...
        public function changeListQuantity(Request $request)
        {
            ...
            else
            {
                ...
                if($quanity_count>0){
                    ...
                    $update_quantity = AddToList::where('id',$list_id)->delete();
                }
                    ...
                    if($id!=""){
                        ...
                        if($count == 0){
                            ...
                            if($check_quantity <= 0){
                                ...
                                $check_product = Addtocart::where('product_id',$product_id)->where('product_quantity_id',$product_quantity_id)->where('customer_id',$id)->where('status',0)->count();
                                $product_price = ProductQuantity::where('product_id',$product_id)->where('id',$product_quantity_id)->pluck('sales_price')->first();
                                $total_price = $product_count * $product_price;
       ...
       /* public function payment(Request $request){
        ...
        public function deliveryFeeCalculation_v1(Request $request){
            $user_lat = $request->lat;
			$user_lng = $request->lng;
			$unit = ($request->unit)?:'K';

            $total_fee = 0;
            for ($i=0; $i < count($request->store_id) ; $i++) { 
                $store = Seller::where('sd_usid',$request->store_id[$i])->select('sd_lat','sd_lng')->first();
                //dump($store);
                if($store->sd_lat){
                    $store_lat = $store->sd_lat;
                    $store_lng = $store->sd_lng;
                    ...
                    $delivery_fee = Deliveryinfo::first();
                    if(($delivery_fee->business) != "off"){
                        $total_fee = $total_fee + $delivery_fee->base_price;
                        if($distance > $delivery_fee->base_price_distance){
                            $total_fee = $total_fee + ($delivery_fee->price_per_unit_distance * ($distance - $delivery_fee->base_price_distance));
                        }
                        if($delivery_fee->service_tax){
        ...
        public function deliveryFeeCalculation_v2(Request $request){            
            $user_lat = $request->lat;
			$user_lng = $request->lng;
			$unit = ($request->unit)?:'K';
            ...
            $final_array1=array();
            for ($i=0; $i < count($request->store_id) ; $i++) { 
                $store = Seller::where('sd_usid',$request->store_id[$i])->select('sd_lat','sd_lng','sd_sname','sd_usid')->first();
                //dump($store);
                $total_fee = 0;
                if($store->sd_lat){
                    $store_lat = $store->sd_lat;
    ...
    public function logout()
    {
        ...
        public function StoreReview(Request $request){
            ...
            if ($token1->valid) {
                if ($token1->existsValid()) {
                    $id=$token1->user_id;
                    if($id!=""){
                        $store_review = Review::select('reviews.*')->where('store_id',$sid)->where('status',1)
                        ->get();
        ...
        public function getProfileImage(Request $request){
            ...
            if ($token1->valid) {
                if ($token1->existsValid()) {
                    $id=$token1->user_id;
                    $ProfileImage=User::select('users.id','users.profile_image')->find($id);
                return response()->json([
                   'status'=> true,
                   'data'=> $ProfileImage,
                   ]);
        ...
        public function getorderreviewlist_v1(Request $request){
            $token=$request->bearerToken();
            $token1 = new PassportToken($token);
            if ($token1->valid) {
                if ($token1->existsValid()) {
        ...
        public function getContactUs(Request $request){
            ...
            if(count($contactus)>0){
                ...
                return response()->json([
                    'status'=>false,
                    'message'=>'Sorry Contact Us not found'
                ]);
            }
        }
        ...
        public function getCityList(Request $request){
            ...
            if(count($City)>0){
                ...
                return response()->json([
                    'status'=>false,
                    'message'=>'Sorry City List not found'
                ]);
            }
        }
        ...
        public function getDeliverySlots(Request $request){
            $DeliverySlots = DeliverySetting::all();
