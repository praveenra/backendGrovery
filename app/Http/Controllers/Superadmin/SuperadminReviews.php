<?php	
	...
	use DB;
	class SuperadminReviews extends Controller	
	{		
		...
		*/		
		...
		public function store_reviews(Request $request){
            $this->data['reviews'] = Review::select('customer.name as customer','reviews.*','seller_details.sd_sname')->leftjoin('customer','customer.id','reviews.customer_id')->leftjoin('seller_details','seller_details.sd_usid','reviews.store_id')->orderBy('reviews.id', 'desc')->get(); //->paginate()
            $this->data['message'] = 'No Review Added';
            $this->data['page_details'] = $this->page_details;
            return view('superadmin.reviews.store_reviews', $this->data);  
		}		
		...
		public function change_all_review_status(Request $request){
			...
			$review = DB::table('reviews')->where('status',NULL)->update(['status' => 1]);
		...
		public function product_reviews(Request $request){			
            ...
            return view('superadmin.reviews.product_reviews', $this->data);  
		}		
		...
		public function change_all_product_review_status(Request $request){
			...
			$review = DB::table('product_reviews')->where('status',NULL)->update(['status' => 1]);
